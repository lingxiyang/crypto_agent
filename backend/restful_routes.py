...
from models.character import Character, CharacterRequest, \
    EditCharacterRequest, DeleteCharacterRequest, GeneratePromptRequest
from models.quivr_info import QuivrInfo, UpdateQuivrInfoRequest
from llm.system_prompt_generator import generate_system_prompt
...
import openai
from utils import (ConversationHistory, build_opemai_history,
...
@router.get("/status")
async def status():
    return {"status": "ok", "message": "crypto agent3 is running smoothly!"}
...
async def generate(messages: List[Dict], model_type: str):
    try:
...
@router.post("/search")
async def search_content(request: ChatRequest,
