...
def get_text_to_speech(tts: str = None) -> TextToSpeech:
    ...
    if tts == 'ELEVEN_LABS':
        ...
        from audio.text_to_speech.google_cloud_tts import GoogleCloudTTS
        GoogleCloudTTS.initialize()
        return GoogleCloudTTS.get_instance()
    elif tts == 'UNREAL_SPEECH':
        from audio.text_to_speech.unreal_speech import UnrealSpeech
        UnrealSpeech.initialize()
        return UnrealSpeech.get_instance()
    elif tts == 'EDGE_TTS':
        from audio.text_to_speech.edge_tts import EdgeTTS
        EdgeTTS.initialize()
